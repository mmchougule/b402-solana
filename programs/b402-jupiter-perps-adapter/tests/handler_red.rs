...
//!
//! `execute()` currently returns `JupPerpsAdapterError::NotYetImplemented`.
//! When the handler ships per the Jupiter-Perps-adapter PRD, this test must
//! be rewritten to assert successful execution against a Jupiter Perps
//! mainnet-fork validator.
//!
...
use b402_jupiter_perps_adapter::JupPerpsAdapterError;

#[test]
fn handler_currently_errors_with_not_yet_implemented() {
    // The error code must be in Anchor's user-error range (>= 6000).
    let err: anchor_lang::error::Error = error!(JupPerpsAdapterError::NotYetImplemented);
    let code = match err {
...
fn implementation_marker_is_present() {
    ...
    let msg = format!(
        "{:?}",
        anchor_lang::error::Error::from(JupPerpsAdapterError::NotYetImplemented)
    );
    assert!(
