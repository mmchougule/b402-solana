...
//!
//! `execute()` currently returns `AdrenaAdapterError::NotYetImplemented`.
//! When the handler ships per PRD-16 §6, this test must be rewritten to
//! assert successful execution against an Adrena mainnet-fork validator.
//!
...
use b402_adrena_adapter::AdrenaAdapterError;

#[test]
fn handler_currently_errors_with_not_yet_implemented() {
    // The error code must be in Anchor's user-error range (>= 6000).
    let err: anchor_lang::error::Error = error!(AdrenaAdapterError::NotYetImplemented);
    let code = match err {
...
fn implementation_marker_is_present() {
    ...
    let msg = format!(
        "{:?}",
        anchor_lang::error::Error::from(AdrenaAdapterError::NotYetImplemented)
    );
    assert!(
