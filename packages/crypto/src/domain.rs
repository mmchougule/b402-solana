...
mod tests {
    ...
    fn all_tags_distinct() {
        let tags = [
            DomainTag::Commit, DomainTag::Nullifier, DomainTag::MerkleNode,
            DomainTag::MerkleZero, DomainTag::NoteEncKey, DomainTag::SpendKey,
            DomainTag::SpendKeyPub, DomainTag::ViewKey, DomainTag::ViewTag,
            DomainTag::FeeBind, DomainTag::RootBind, DomainTag::AdaptBind,
            DomainTag::Disclose,
        ];
        let frs: Vec<Fr> = tags.iter().map(|t| t.to_fr()).collect();
        for i in 0..frs.len() {
            for j in (i + 1)..frs.len() {
                assert_ne!(frs[i], frs[j], "tags {:?} and {:?} collide", tags[i], tags[j]);
            }
        }
    }
}
