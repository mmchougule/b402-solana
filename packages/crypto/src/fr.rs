    ...
    pub fn from_le_bytes(bytes: &[u8]) -> Result<Self, FrError> {
        ...
        for i in (0..32).rev() {
            match bytes[i].cmp(&modulus_le[i]) {
                std::cmp::Ordering::Less => { is_canonical = true; break; }
                std::cmp::Ordering::Greater => { is_canonical = false; break; }
...
mod tests {
    ...
    fn rejects_non_canonical() {
        ...
        let p_minus_1 = [
            0x00, 0x00, 0x00, 0xf0, 0x93, 0xf5, 0xe1, 0x43,
            0x91, 0x70, 0xb9, 0x79, 0x48, 0xe8, 0x33, 0x28,
            0x5d, 0x58, 0x81, 0x81, 0xb6, 0x45, 0x50, 0xb8,
            0x29, 0xa0, 0x31, 0xe1, 0x72, 0x4e, 0x64, 0x30,
        ];
        ...
        assert!(Fr::from_le_bytes(&p_minus_1).is_ok(), "p-1 should be canonical");
        assert!(Fr::from_le_bytes(&p).is_err(), "p itself should be rejected");
    }
}
