#!/usr/bin/env node
/**
 * Generate a known-good adapt v2 proof for Rust verifier tests.
 *
 * Produces `build/test_artifacts/adapt_v2_valid.json` containing:
 *   proof_a_be:    [64]byte hex (big-endian, groth16-solana format)
 *   proof_b_be:    [128]byte hex
 *   proof_c_be:    [64]byte hex
 *   public_inputs_be: [38] 32-byte BE hex strings
 *   public_decimals:  [38] decimal strings (for debugging)
 *
 * Scenario: 1-in / 1-out shielded swap-style action.
 *   Alice has 100 IN_MINT shielded (one input note at leaf 0).
 *   Pool sends 80 IN_MINT to adapter; 20 is the relayer fee.
 *   Adapter delivers 250 OUT_MINT, all as one output note for Alice.
 *   Shadow PDA active (Kamino-style scope).
...
const WASM = path.join(BUILD, 'adapt_v2_js/adapt_v2.wasm');
const ZKEY = path.join(BUILD, 'ceremony/adapt_v2_final.zkey');
...
const tags = {
  ...
  adaptBindV2:   tagToFr('b402/v2/adapt-bind'),
  shadowBind:    tagToFr('b402/v2/shadow-bind'),
};
...
async function buildAdaptV2Input() {
  const aliceSp = 42n;
  ...
  // Sample canonical accounts hash (3 dummy accounts).
  const accountsCanonical = new Uint8Array(33 * 3);
  for (let i = 0; i < 3; i++) {
    for (let b = 0; b < 32; b++) accountsCanonical[i * 33 + b] = i + 1;
    accountsCanonical[i * 33 + 32] = i % 3; // flags
  }
  const accountsHash = leToFrReduced(keccak_256(accountsCanonical));

  const scopeTag = tagToFr('kamino:lend:v1');
  const extraContextRoot = 0n;

  const actionHash = await poseidon(
    tags.adaptBindV2, adapterId, scopeTag,
    actionPayloadKeccakFr, accountsHash, extraContextRoot,
  );

  // Shadow PDA active.
  const viewingPubHash = await poseidon(aliceSpPub, 0n);
  const shadowPdaBinding = await poseidon(tags.shadowBind, viewingPubHash, scopeTag);
  ...
  return {
    ...
    nullifier: [inNullifier.toString(), '0', '0', '0'],
    commitmentOut: [outCommit.toString(), '0', '0', '0'],
    publicAmountIn: publicAmountIn.toString(),
    publicAmountOut: '0',
    publicTokenMintIn: [inMint.toString(), '0', '0', '0'],
    ...
    expectedOutMint: [outMint.toString(), '0', '0', '0'],
    adaptBindTag: tags.adaptBindV2.toString(),
    scopeTag: scopeTag.toString(),
    accountsHash: accountsHash.toString(),
    extraContextRoot: extraContextRoot.toString(),
    deadlineSlot: deadlineSlot.toString(),
    shadowPdaBinding: shadowPdaBinding.toString(),
    inTokenMint: [inMint.toString(), '0', '0', '0'],
    inValue: [inValue.toString(), '0', '0', '0'],
    inRandom: [inRandom.toString(), '0', '0', '0'],
    inSpendingPriv: [aliceSp.toString(), dummyPriv.toString(), dummyPriv.toString(), dummyPriv.toString()],
    inLeafIndex: ['0', '0', '0', '0'],
    inSiblings: [
      zero.slice(0, 26).map(String),
      zero.slice(0, 26).map(String),
      zero.slice(0, 26).map(String),
      zero.slice(0, 26).map(String),
    ],
    inPathBits: [Array(26).fill('0'), Array(26).fill('0'), Array(26).fill('0'), Array(26).fill('0')],
    inIsDummy: ['0', '1', '1', '1'],
    outTokenMint: [outMint.toString(), '0', '0', '0'],
    outValue: [outValue.toString(), '0', '0', '0'],
    outRandom: [outRandom.toString(), '0', '0', '0'],
    outSpendingPub: [aliceSpPub.toString(), '0', '0', '0'],
    outIsDummy: ['0', '1', '1', '1'],
    ...
    shadowDomainTag: tags.shadowBind.toString(),
    viewingPubHash: viewingPubHash.toString(),
  };
}
...
async function main() {
  if (!fs.existsSync(WASM) || !fs.existsSync(ZKEY)) {
    console.error('missing artifacts. compile adapt_v2.circom + run adapt v2 ceremony first.');
    process.exit(1);
  }

  const input = await buildAdaptV2Input();
  const { proof, publicSignals } = await snarkjs.groth16.fullProve(input, WASM, ZKEY);

  const vKey = JSON.parse(fs.readFileSync(path.join(BUILD, 'ceremony/adapt_v2_verification_key.json'), 'utf8'));
  ...
  const out = {
    note: 'Generated from a throwaway-ceremony adapt v2 zkey. DEVNET / LOCAL ONLY.',
    proof_a_be: proofABe,
    proof_b_be: proofBBe,
    proof_c_be: proofCBe,
  ...
  const outPath = path.join(OUT_DIR, 'adapt_v2_valid.json');
