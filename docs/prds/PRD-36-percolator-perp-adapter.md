...
| **4-α** | SDK encoding — `packages/sdk/src/percolator.ts` with action-payload + per-user wrapper + execute-ix builders, three PDA derivations, RA-layout builder. Byte-pinned to Rust via shared `FIXTURE_*_HEX` (encoder drift on either side breaks both test suites). | **DONE** | (next commit) | +23 SDK + 4 Rust fixture pins |
| 4-β | SDK methods proper — `B402Solana.privatePerpOpen` / `privatePerpClose` (call into Phase-9 prover, build the adapt_execute tx, wire ALT for the variadic remaining_accounts). | next | — | — |
| 4-γ | Mapping-account init helper + a one-shot script for SDK consumers (`pnpm percolator:init-mapping <slab>`). Done lazily on first open per market, but standalone helper makes ops easier. | blocked on 4-β | — | — |
