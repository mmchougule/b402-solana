...
describe('PRD-33 Phase 33.3 — per-user Kamino obligation isolation (mainnet-fork)', () => {
  it(
    ...
    async () => {
      ...
      // V1: rent funded via the SystemProgram.transfer 5 SOL bootstrap
      // top-up below to adapter_authority. PRD-33 §5.4's per-deposit fee
      // path is V1.5 — gated on Phase 8 ALT auto-builder (per-call extender)
      // since the kamino-deposit tx already sits at the 1232 B v0-tx cap
      // with Phase 9's 24 public inputs + 5 forced-static per-user PDAs.
      ...
      const SHIELD_AMT = 1_000_000n; // 1 USDC each (V1 — no per-deposit fee)
