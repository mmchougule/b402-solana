 ...
 *
 * Adapted from phase9_localnet_dual_note.test.ts. Same setup, but the
 * privateSwap call sets `pendingInputsMode: true` to exercise:
 *   - SDK builds + submits commit_inputs tx 1 → writes 24×32 B inputs
 *     into the per-user pending_inputs PDA, version = 1.
 *   - SDK then submits adapt_execute tx 2 — references the PDA in the
 *     account list. Pool's adapt_execute validates the PDA derivation,
 *     byte-equals the inputs against its locally-recomputed vector,
 *     CPIs verifier::verify_with_account_inputs, then zeroes the
 *     version byte for replay protection.
 *
 * Asserts the same Phase 9 dual-note round-trip works under PRD-35
 * (excess > 0 mint + spend) AND the pending_inputs PDA exists with
 * version = 0 after the swap.
 *
 * Pre-conditions (caller must satisfy):
 *   - Light test-validator running with binaries built with the new
 *     features:
 ...
 *   - Test enabled: B402_FORK_PRD_35=1
 */
...
describe.skipIf(process.env.B402_FORK_PRD_35 !== '1')('PRD-35 §5.6 — pending-inputs 2-tx flow on localnet', () => {
  it('shield → commit_inputs → adapt_execute (reads PDA) → version zeroed → unshield works', async () => {
    ...
    // PRD-35 §5.4 — extend the ALT with the user's pending_inputs PDA
    // so it compresses to 1 B in tx 2 instead of 32 B static. Without
    // this, even a simple swap would push past the cap once any other
    // per-user state is added downstream (Kamino etc).
    await b402.ready();
    const spendingPubLeBuf = Buffer.alloc(32);
    {
      let v = b402.wallet.spendingPub;
      for (let i = 0; i < 32; i++) { spendingPubLeBuf[i] = Number(v & 0xffn); v >>= 8n; }
    }
    const [pendingInputsPda] = derivePendingInputsPda(POOL_ID, spendingPubLeBuf);
    const extendIx2 = AddressLookupTableProgram.extendLookupTable({
      ...
      addresses: [pendingInputsPda],
    });
    await sendAndConfirmTransaction(conn, new Transaction().add(extendIx2), [admin], { commitment: 'confirmed' });
    await new Promise((r) => setTimeout(r, 3000));
    ...
    const swapRes = await b402.privateSwap({
      ...
      actionPayload,
      phase9DualNote: true,
      pendingInputsMode: true,                // ← PRD-35 2-tx flow ON
    });

    // PRD-35 ASSERTION 1 — pending_inputs PDA exists with version=0
    // (committed by tx 1, then zeroed by tx 2's post-verify cleanup).
    const pendingAcct = await conn.getAccountInfo(pendingInputsPda);
    expect(pendingAcct, 'pending_inputs PDA must exist post-swap').not.toBeNull();
    expect(pendingAcct!.data.length).toBeGreaterThanOrEqual(8 + 1 + 32 * 24);
    expect(pendingAcct!.data[8]).toBe(0); // version zeroed = replay protected
