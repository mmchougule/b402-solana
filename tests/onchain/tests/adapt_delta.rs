...
fn adapter_authority() -> Pubkey {
    Pubkey::find_program_address(&[b"b402/v1", b"adapter"], &mock_adapter_id()).0
}
...
fn setup_svm() -> (LiteSVM, Accounts, Keypair) {
    ...
    // Out side — what the mock "swaps into".
    ...
    let adapter_out_ta = Pubkey::new_from_array([9u8; 32]);
    mint::plant_token_account(&mut svm, &adapter_out_ta, &out_mint, &auth, 10_000);

    // In side — mock requires adapter_in_ta to have `in_amount` tokens
    // (unified ABI check, mirrors what pool pre-transfers in real flow).
    let in_mint = Pubkey::new_from_array([11u8; 32]);
    mint::plant_mint(&mut svm, &in_mint, &caller.pubkey(), 6);
    let in_vault = Pubkey::new_from_array([12u8; 32]);
    mint::plant_token_account(&mut svm, &in_vault, &in_mint, &caller.pubkey(), 0);
    let adapter_in_ta = Pubkey::new_from_array([13u8; 32]);
    mint::plant_token_account(&mut svm, &adapter_in_ta, &in_mint, &auth, 10_000);

    (svm, Accounts { out_vault, adapter_out_ta, in_vault, adapter_in_ta }, caller)
}
...
fn send_check(
    ...
    //   adapter_authority, in_vault, out_vault, adapter_in_ta, adapter_out_ta, token_program.
    let ix = Instruction {
...
fn adapter_returning_exactly_min_out_passes_check() {
    ...
    send_check(&mut svm, &caller, &accts, 50, 100, 0)
        .expect("delta=0 must pass");

    use spl_token::solana_program::program_pack::Pack;
    let v = svm.get_account(&accts.out_vault).unwrap().data;
...
fn adapter_returning_more_than_min_out_passes_check() {
    ...
    send_check(&mut svm, &caller, &accts, 50, 100, 25)
        .expect("delta>0 must pass");
...
fn adapter_returning_less_than_min_out_is_rejected() {
    ...
    let result = send_check(&mut svm, &caller, &accts, 50, 100, -1);
